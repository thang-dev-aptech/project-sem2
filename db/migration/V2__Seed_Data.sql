...
INSERT INTO roles(name, description)
SELECT 'STAFF','Nhân viên quầy - Quyền hạn chế (40%)'
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name='STAFF');

-- OWNER giữ lại để tương thích với dữ liệu cũ (tương đương ADMIN)
INSERT INTO roles(name, description)
SELECT 'OWNER','Chủ phòng gym (tương đương ADMIN)'
...
-- Grant OWNER role to admin user (backward compatibility)
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
...
-- Grant STAFF role to staff user
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
...
-- Grant OWNER role to manager user (backward compatibility)
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
JOIN roles r ON r.name = 'OWNER'
...
-- Grant ADMIN role to manager user
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
JOIN roles r ON r.name = 'ADMIN'
